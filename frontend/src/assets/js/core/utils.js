...
export function getDesktopPath() {
    if (window.process?.env?.USERPROFILE) { // Windows
        return `${window.process.env.USERPROFILE}/Desktop/ReceivedFiles`;
    }
    return `${window.require('os').homedir()}/Desktop/ReceivedFiles`; // Mac/Linux
}
...
export async function copyToClipboard(text) {
  try {
    // Современный API (работает на HTTPS и localhost)
    ...
    textArea.value = text;
    textArea.style.position = 'fixed'; // Убрать из вьюпорта
    document.body.appendChild(textArea);
    textArea.select();
    
    // iOS требует явного вызова select()
